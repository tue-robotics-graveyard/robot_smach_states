    ...
    def __init__(self, robot, initial_pose, use_entry_points=False):
        ...
        with self:
            ...
            smach.StateMachine.add("INSTRUCT_WAIT_FOR_DOOR",
                                   human_interaction.Say(robot, ["Hi there, I will now wait until the door is "
                                                                 "opened", "I'm waiting for the door"], block=False),
            ...
            #  since it is thinks amigo is standing in front of a wall if door is closed and localization can(/will)
            # be messed up.
            smach.StateMachine.add('INIT_POSE',
                                   utility.SetInitialPose(robot, initial_pose),
                                   transitions={'done': 'ENTER_ROOM',
                                                'preempted': 'Aborted',  # This transition will never happen
    ...
    def __init__(self, robot, initial_pose, use_entry_points=False):
        ...
        with self:
            ...
            smach.StateMachine.add("THROUGH_DOOR",
                                   human_interaction.Say(robot, ["I will start my task now",
                                                                 "Let's rock and roll!",
...
class WaitForDoorOpen(State):
    ...
    def process_scan(self, scan_msg):
        try:
            ...
            distance_to_door = self.avg(ranges_at_center)  # and the average of the middle range and use it as the
