...
class Initialize(smach.State):
    ...
    def execute(self, userdata):
        self.robot.lights.set_color(0, 0, 1)  # Make sure lights are blue
        ...
        # Check if TF link between /map and /base_link is set, if not error at initialize in stead of during first
        # navigate execution
        rospy.loginfo("TF link between /map and /base_link is checked. If it takes longer than a second, probably an "
                      "error. Do a restart!!!")
...
#         self.robot.head.look_at_hand(side_string, keep_tracking=self.keep_tracking)
#         # TODO: Unify side as string or/and object
#         return "done"
...
class LockDesignator(smach.State):
    ...
    def execute(self, userdata=None):
        ...
        rospy.loginfo("locking_designator {1} is now locked to {0}".format(str(self.locking_designator.resolve())[:10],
                                                                           self.locking_designator))
        return 'locked'
...
class UnlockDesignator(smach.State):
    ...
    def execute(self, userdata=None):
        rospy.loginfo("locking_designator {1} is going to unlock from "
                      "{0}".format(str(self.locking_designator.resolve())[:10], self.locking_designator))
